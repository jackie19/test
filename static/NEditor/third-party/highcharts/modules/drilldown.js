;(function (e) {
  function q(b, a, c) {
    return (
      'rgba(' +
      [
        Math.round(b[0] + (a[0] - b[0]) * c),
        Math.round(b[1] + (a[1] - b[1]) * c),
        Math.round(b[2] + (a[2] - b[2]) * c),
        b[3] + (a[3] - b[3]) * c,
      ].join(',') +
      ')'
    )
  ...
  e.SVGRenderer.prototype.Element.prototype.fadeIn = function () {
    this.attr({ opacity: 0.1, visibility: 'visible' }).animate(
      { opacity: 1 },
      { duration: 250 }
    )
  }
  ...
  h.prototype.drillUp = function () {
    ...
    if (d.type === a.type)
      (d.drilldownLevel = b),
        ...
        a.animateDrillupFrom && a.animateDrillupFrom(b)
    a.remove(!1)
    d.xAxis &&
      (d.xAxis.setExtremes(c.xMin, c.xMax, !1),
      d.yAxis.setExtremes(c.yMin, c.yMax, !1))
    this.redraw()
  ...
  k.prototype.animateDrilldown = function (b) {
    ...
    b ||
      g(this.points, function (a, b) {
        ...
        a.graphic
          .attr(e.merge(d, { start: f + b * s, end: f + (b + 1) * s }))
          .animate(
            a.shapeArgs,
            e.merge(c, {
              step: function (a, d) {
  ...
  l.prototype.animateDrillupFrom = k.prototype.animateDrillupFrom = function (
    b
  ) {
    ...
    j = function (b) {
      ...
      g(this.points, function (a) {
        a.drilldown &&
          a.graphic &&
          a.graphic
            .attr({ class: 'highcharts-drilldown-point' })
            .css({ cursor: 'pointer' })
      })
    }
  for (p in i)
