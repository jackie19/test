  ...
  seriesTypes.map = Highcharts.extendClass(seriesTypes.scatter, {
    ...
    drawLegendSymbolGradient: function (legend, item) {
      var spacing = legend.options.symbolPadding,
        ...
        positionY,
        positionX,
        gradientSize = this.chart.renderer.fontMetrics(
          legend.options.itemStyle.fontSize
        ).h,
      ...
      item.legendSymbol = this.chart.renderer
        .rect(
          horizontal
            ? box1.x + box1.width + spacing
            : box1.x - gradientSize - spacing, // Upper left x.
          box1.y, // Upper left y.
      ...
      if (horizontal) {
        legend.offsetWidth =
          box1.width + box2.width + box3.width + spacing * 2 + padding
        legend.itemY = gradientSize + padding
      } else {
        legend.offsetWidth =
          Math.max(box1.width, box3.width) + spacing + box2.width + padding
    ...
    drawPoints: function () {
      ...
      each(series.data, function (point) {
        ...
        point.plotX = Math.round(
          minX + (maxX - minX) * pick(dataLabels && dataLabels.anchorX, 0.5)
        )
        point.plotY = Math.round(
          minY + (maxY - minY) * pick(dataLabels && dataLabels.anchorY, 0.5)
        )
