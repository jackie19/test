  ...
  function build() {
    ...
    svg.AspectRatio = function (
      ...
      if (refX.hasValue() && refY.hasValue()) {
        ctx.translate(
          -scaleMin * refX.Length.toPixels('x'),
          -scaleMin * refY.Length.toPixels('y')
        )
      } else {
        // align
        if (
      ...
      this.setContext = function (ctx) {
        ...
        if (this.style('fill').Definition.isUrl()) {
          ...
          if (this.style('fill-opacity').hasValue())
            fillStyle = fillStyle.Color.addOpacity(
        ...
        if (this.style('stroke').Definition.isUrl()) {
          ...
          if (this.style('stroke-opacity').hasValue())
            strokeStyle = strokeStyle.Color.addOpacity(
          ...
          switch (pp.command.toUpperCase()) {
            ...
            case 'A':
              while (!pp.isCommandOrEnd()) {
                ...
                pp.addMarkerAngle(
                  halfWay,
                  (a1 + ad) / 2 + ((sweepFlag == 0 ? 1 : -1) * Math.PI) / 2
                )
                pp.addMarkerAngle(
                  cp,
                  ad + ((sweepFlag == 0 ? 1 : -1) * Math.PI) / 2
                )
    ...
    svg.Element.text = function (node) {
      ...
      this.renderChildren = function (ctx) {
        ...
        for (var i = 0; i < this.children.length; i++) {
          ...
          if (
            textAnchor != 'start' &&
            (i == 0 || child.attribute('x').hasValue())
          ) {
            // new group?
      ...
      this.renderChildren = function (ctx) {
        if (this.hasText) {
          ...
          svg.Mouse.checkBoundingBox(
            ...
            new svg.BoundingBox(
              this.x,
              this.y - fontSize.Length.toPixels('y'),
    ...
    svg.Element.style = function (node) {
      ...
      var css =
        ...
        (node.childNodes.length > 1 ? node.childNodes[1].nodeValue : '')
      css = css.replace(
        /(\/\*([^*]|[\r\n]|(\*+([^*\/]|[\r\n])))*\*+\/)|(^[\s]*\/\/.*)/gm,
        ''
      ) // remove comments
        ...
        for (var y = 0; y < height; y++) {
          for (var x = 0; x < width; x++) {
            ...
            for (var rgba = 0; rgba < 4; rgba++) {
              var sum =
                mask[0] *
                (a == 0 ? 255 : imGet(src, x, y, width, height, rgba)) *
                (a == 0 || rgba == 3 ? 1 : a)
              for (var i = 1; i < mask.length; i++) {
                var a1 =
                  imGet(src, Math.max(x - i, 0), y, width, height, 3) / 255
                var a2 =
                  imGet(src, Math.min(x + i, width - 1), y, width, height, 3) /
                  255
                sum +=
                  mask[i] *
                  ((a1 == 0
                    ? 255
                    : imGet(src, Math.max(x - i, 0), y, width, height, rgba)) *
                    (a1 == 0 || rgba == 3 ? 1 : a1) +
                    (a2 == 0
                      ? 255
                      : imGet(
                          src,
                          Math.min(x + i, width - 1),
                          y,
                          width,
                          height,
                          rgba
      ...
      var draw = function () {
        ...
        if (
          ...
          if (e.attribute('width').hasValue())
            xRatio =
              e.attribute('width').Length.toPixels('x') / svg.opts['scaleWidth']
          if (e.attribute('height').hasValue())
            yRatio =
              e.attribute('height').Length.toPixels('y') /
        ...
        if (isFirstRender) {
          ...
          if (
            svg.opts != null &&
            typeof svg.opts['renderCallback'] == 'function'
          )
