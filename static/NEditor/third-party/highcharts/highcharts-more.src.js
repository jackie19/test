...
;(function (Highcharts, UNDEFINED) {
  ...
  var radialAxisMixin = {
    ...
    setAxisSize: function () {
      ...
      if (this.isRadial) {
        ...
        this.center = this.pane.center = seriesTypes.pie.prototype.getCenter.call(
          this.pane
        )
    ...
    getTitlePosition: function () {
      ...
      return {
        ...
        y:
          chart.plotTop +
          center[1] -
          { high: 0.5, middle: 0.25, low: 0 }[titleOptions.align] * center[2] +
          (titleOptions.y || 0),
      }
    },
  }
  ...
  wrap(axisProto, 'init', function (proceed, chart, userOptions) {
    ...
    if (!isHidden && (angular || polar)) {
      ...
      this.pane = pane = chart.panes[paneIndex] =
        chart.panes[paneIndex] ||
        new Pane(splat(chartOptions.pane)[paneIndex], chart, axis)
  ...
  wrap(tickProto, 'getLabelPosition', function (
    ...
    if (axis.isRadial) {
      ...
      if (labelOptions.rotation === 'auto') {
        ...
        optionsY =
          pInt(label.styles.lineHeight) * 0.9 - label.getBBox().height / 2
      }
   ...
   */
  defaultPlotOptions.columnrange = merge(
  ...
  seriesTypes.waterfall = extendClass(seriesTypes.column, {
    ...
    getAttribs: function () {
      ...
      seriesDownPointAttr.hover[upColorProp] =
        stateOptions.hover.upColor || hoverColor
      seriesDownPointAttr.select[upColorProp] =
        stateOptions.select.upColor || upColor
  ...
  Axis.prototype.beforePadding = function () {
    ...
    if (this.tickPositions) {
      ...
      if (
        activeSeries.length &&
        range > 0 &&
        pick(this.options.min, this.userMin) === UNDEFINED &&
        pick(this.options.max, this.userMax) === UNDEFINED
      ) {
        pxMax -= axisLength
  ...
  seriesProto.toXY = function (point) {
    ...
    point.clientX =
      ((plotX / Math.PI) * 180 + this.xAxis.pane.options.startAngle) % 360
  ...
  wrap(seriesTypes.spline.prototype, 'getPointSpline', function (
    ...
    if (this.chart.polar) {
      ...
      if (lastPoint && nextPoint) {
        ...
        distanceLeftControlPoint = Math.sqrt(
          Math.pow(leftContX - plotX, 2) + Math.pow(leftContY - plotY, 2)
        )
        distanceRightControlPoint = Math.sqrt(
        ...
        rightContX =
          plotX + Math.cos(Math.PI + jointAngle) * distanceRightControlPoint
        rightContY =
          plotY + Math.sin(Math.PI + jointAngle) * distanceRightControlPoint
  ...
  wrap(seriesProto, 'getSegmentPath', function (proceed, segment) {
    ...
    if (
      ...
      segment[segment.length - 1] === points[points.length - 1] &&
      points[0].y !== null
    ) {
