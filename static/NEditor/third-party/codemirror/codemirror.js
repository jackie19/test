...
var CodeMirror = (function () {
  ...
  function CodeMirror(place, givenOptions) {
      ...
      addWidget: function (pos, node, scroll, vert, horiz) {
        ...
        if (vert == 'over') top = pos.y
          ...
          var vspace = Math.max(
              ...
              doc.height * textHeight()
            ),
            hspace =
              Math.max(code.clientWidth, lineSpace.clientWidth) - paddingLeft()
          if (
            pos.yBot + node.offsetHeight > vspace &&
            pos.y > node.offsetHeight
          )
      ...
      setLine: operation(function (line, text) {
        if (isLine(line))
          replaceRange(
            text,
            { line: line, ch: 0 },
            { line: line, ch: getLine(line).text.length }
          )
      }),
    ...
    function setValue(code) {
      ...
      updateLines(
        top,
        { line: doc.size - 1, ch: getLine(doc.size - 1).text.length },
        splitLines(code),
        top,
        top
      )
    ...
    function unredoHelper(from, to) {
      ...
      if (change) {
        ...
        updateLinesNoUndo(
          { line: change.start, ch: 0 },
          { line: end - 1, ch: getLine(end - 1).text.length },
          change.old,
          pos,
          pos
        )
      ...
      function adjustPos(pos) {
        ...
        if (pos.line == to.line)
          ch +=
            ...
            (to.ch - (to.line == from.line ? from.ch : 0))
    ...
    function updateGutter() {
      ...
      var html = [],
        i = showingFrom
      doc.iter(showingFrom, Math.max(showingTo, showingFrom + 1), function (
        line
      ) {
        if (line.hidden) {
          html.push('<pre></pre>')
      ...
      if (posEq(sel.from, sel.to)) {
        ...
        cursor.style.left =
          (options.lineWrapping
            ? Math.min(pos.x, lineSpace.offsetWidth)
            : pos.x) + 'px'
    ...
    function findPosH(dir, unit) {
      ...
      function findNextLine() {
        for (
          var l = line + dir, e = dir < 0 ? -1 : doc.size;
          l != e;
          l += dir
        ) {
          var lo = getLine(l)
    ...
    function indentLine(n, how) {
      ...
      replaceRange(
        indentString,
        { line: n, ch: 0 },
        { line: n, ch: curSpaceString.length }
      )
    }
    ...
    function themeChanged() {
      scroller.className =
        scroller.className.replace(/\s*cm-s-\w+/g, '') +
      ...
      return changeLine(handle, function (line, no) {
        if (line.hidden != hidden) {
          ...
          if (hidden && (sel.from.line == no || sel.to.line == no))
            setSelection(
              skipHidden(sel.from, sel.from.line, sel.from.ch),
    ...
    function measureLine(line, ch) {
      ...
      if (options.lineWrapping) {
        ...
        extra = htmlEscape(
          line.text.slice(
            ch + 1,
            end < 0 ? line.text.length : end + (ie ? 5 : 0)
          )
        )
      }
      measure.innerHTML =
    ...
    function localCoords(pos, inLineWrap) {
      var x,
        lh = textHeight(),
        y =
          lh * (heightAtLine(doc, pos.line) - (inLineWrap ? displayOffset : 0))
      if (pos.ch == 0) x = 0
      else {
        ...
        doc.iter(i, doc.size, function (line) {
          ...
          if (compare) {
            ...
            if (changed !== false || !hadState) unchanged = 0
              ...
              ++unchanged > 3 &&
              (!mode.indent ||
                mode.indent(hadState, '') == mode.indent(state, ''))
            )
              return true
          }
          ++i
  ...
  MarkedText.prototype = {
    ...
    split: function (pos, lenBefore) {
      ...
      var from =
        this.from < pos || this.from == null
          ? null
          : this.from - pos + lenBefore
  ...
  Line.prototype = {
    ...
    highlight: function (mode, state, tabSize) {
      ...
      while (!stream.eol()) {
        ...
        if (pos && st[pos - 1] == style) st[pos - 2] += substr
          ...
          if (
            !changed &&
            (st[pos + 1] != style || (pos && st[pos - 2] != prevWord))
          )
            changed = true
          st[pos++] = substr
          st[pos++] = style
    ...
    getHTML: function (sfrom, sto, includePre, tabText, endAt) {
      ...
      if (includePre)
        html.push(
          this.className ? '<pre class="' + this.className + '">' : '<pre>'
        )
      function span(text, style) {
        ...
        if (style)
          html.push(
            ...
            style,
            '">',
            htmlEscape(text).replace(/\t/g, tabText),
            '</span>'
          )
  ...
  function copyStyles(from, to, source, dest) {
    for (var i = 0, pos = 0, state = 0; pos < to; i += 2) {
      ...
      if (state == 0) {
        if (end > from)
          dest.push(
            part.slice(from - pos, Math.min(part.length, to - pos)),
            source[i + 1]
          )
        if (end >= from) state = 1
      } else if (state == 1) {
    ...
    '\n\nb'.split(/\n/).length != 3
      ? function (string) {
          ...
          while ((nl = string.indexOf('\n', pos)) > -1) {
            result.push(
              string.slice(pos, string.charAt(nl - 1) == '\r' ? nl - 1 : nl)
            )
            pos = nl + 1
          }
