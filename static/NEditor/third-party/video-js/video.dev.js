  ...
  init: function (player, options, ready) {
    ...
    this.id_ =
      ...
      (options['el'] && options['el']['id']
        ? options['el']['id']
        : player.id() + '_component_' + vjs.guid++)
...
vjs.Component.prototype.addChild = function (child, options) {
  ...
  if (typeof child === 'string') {
    ...
    component = new window['videojs'][componentClass](
      this.player_ || this,
      options
    )
 ...
 * @private
 */
vjs.Component.prototype.dimension = function (
  widthOrHeight,
  num,
  skipListeners
) {
...
vjs.DurationDisplay.prototype.updateContent = function () {
  ...
  if (duration) {
    this.content.innerHTML =
      ...
      vjs.formatTime(duration) // label the duration time for screen reader users
  }
}
...
vjs.RemainingTimeDisplay.prototype.createEl = function () {
  ...
  this.content = vjs.createEl('div', {
    ...
    innerHTML:
      '<span class="vjs-control-text">Remaining Time </span>' + '-0:00', // label the remaining time for screen reader users
    'aria-live': 'off', // tell screen readers not to automatically read the time as it changes
  })
...
vjs.VolumeBar.prototype.updateARIAAttributes = function () {
  ...
  this.el_.setAttribute(
    'aria-valuenow',
    vjs.round(this.player_.volume() * 100, 2)
  )
  this.el_.setAttribute(
    'aria-valuetext',
    vjs.round(this.player_.volume() * 100, 2) + '%'
...
vjs.VolumeMenuButton.prototype.createMenu = function () {
  ...
  var vc = new vjs.VolumeBar(
    this.player_,
    vjs.obj.merge({ vertical: true }, this.options_.volumeBar)
  )
  menu.addChild(vc)
  return menu
}
...
vjs.Html5 = vjs.MediaTechController.extend({
  ...
  init: function (player, options, ready) {
    ...
    if (
      vjs.TOUCH_ENABLED &&
      player.options()['nativeControlsForTouch'] !== false
    ) {
...
vjs.Html5.prototype.setupTriggers = function () {
  for (var i = vjs.Html5.Events.length - 1; i >= 0; i--) {
    vjs.on(
      this.el_,
      vjs.Html5.Events[i],
      vjs.bind(this.player_, this.eventHandler)
    )
  }
}
...
vjs.TextTrack = vjs.Component.extend({
  ...
  init: function (player, options) {
    ...
    this.id_ =
      options['id'] ||
      'vjs_' + options['kind'] + '_' + options['language'] + '_' + vjs.guid++
...
vjs.TextTrack.prototype.update = function () {
  if (this.cues_.length > 0) {
    ...
    if (
      this.prevChange === undefined ||
      time < this.prevChange ||
      this.nextChange <= time
    ) {
      var cues = this.cues_,
