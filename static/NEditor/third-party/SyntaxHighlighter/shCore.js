...
;(function (undefined) {
  ...
  XRegExp = function (pattern, flags) {
    ...
    while (pos < pattern.length) {
      ...
      if (tokenResult) {
        ...
        if (
          (match = nativ.exec.call(nativeTokens[currScope], pattern.slice(pos)))
        ) {
          output.push(match[0])
          pos += match[0].length
        } else {
  ...
  String.prototype.replace = function (search, replacement) {
    ...
    if (Object.prototype.toString.call(replacement) === '[object Function]') {
      ...
      result = nativ.replace.call(str, search, function () {
        ...
        return nativ.replace.call(replacement + '', replacementToken, function (
          $0,
          $1,
          $2
        ) {
  ...
  function clone(regex, additionalFlags) {
    ...
    regex = XRegExp(
      regex.source,
      getNativeFlags(regex) + (additionalFlags || '')
    )
    if (x) {
      regex._xregexp = {
        source: x.source,
  ...
  var SyntaxHighlighter = (function () {
    ...
    var sh = {
      ...
      toolbar: {
        ...
        getHtml: function (highlighter) {
          ...
          function defaultGetHtml(highlighter, name) {
            return sh.toolbar.getButtonHtml(
              highlighter,
              name,
              sh.config.strings[name]
            )
          }

          for (var i = 0; i < list.length; i++)
            html += (items[list[i]].getHtml || defaultGetHtml)(
              highlighter,
              list[i]
            )
        ...
        getButtonHtml: function (highlighter, commandName, label) {
          return (
            '<span><a href="#" class="toolbar_item' +
            ' command_' +
            commandName +
            ' ' +
            commandName +
        ...
        handler: function (e) {
          ...
          var highlighter = getHighlighterById(
              findParentElement(target, '.syntaxhighlighter').id
            ),
          ...
          expandSource: {
            getHtml: function (highlighter) {
              ...
              return sh.toolbar.getButtonHtml(
                highlighter,
                'expandSource',
                title ? title : sh.config.strings.expandSource
              )
            },
      ...
      findElements: function (globalParams, element) {
        var elements = element
            ? [element]
            : toArray(document.getElementsByTagName(sh.config.tagName)),
          conf = sh.config,
          result = []
    ...
    function findElement(target, search, reverse /* optional */) {
      ...
      var nodes = reverse != true ? target.childNodes : [target.parentNode],
        propertyToFind =
          { '#': 'id', '.': 'className' }[search.substr(0, 1)] || 'nodeName',
        expectedValue,
        found

      expectedValue =
        propertyToFind != 'nodeName' ? search.substr(1) : search.toUpperCase()
    ...
    sh.Highlighter.prototype = {
      ...
      getLineNumbersHtml: function (code, lineNumbers) {
        ...
        for (var i = 0; i < count; i++) {
          var lineNumber = lineNumbers ? lineNumbers[i] : firstLine + i,
            code =
              lineNumber == 0 ? sh.config.space : padNumber(lineNumber, pad)
      ...
      getMatchesHtml: function (code, matches) {
        ...
        for (var i = 0; i < matches.length; i++) {
          ...
          result +=
            wrapLinesWithCode(
              code.substr(pos, match.index - pos),
        ...
        result += wrapLinesWithCode(
          code.substr(pos),
          getBrushNameCss() + 'plain'
        )
      ...
      getHtml: function (code) {
        ...
        html =
          ...
          '<tr>' +
          (gutter
            ? '<td class="gutter">' + this.getLineNumbersHtml(code) + '</td>'
            : '') +
          '<td class="code">' +
          '<div class="container">' +
          html +
...
;(function () {
  ...
  function Brush() {
    ...
    var commenthelp =
      ' component description example externalhelp forwardhelpcategory forwardhelptargetname forwardhelptargetname functionality inputs link notes outputs parameter remotehelprunspace role synopsis'

    this.regexList = [
      {
        regex: new RegExp(
          '^\\s*#[#\\s]*\\.(' + this.getKeywords(commenthelp) + ').*$',
          'gim'
        ),
      ...
      {
        regex: new RegExp(
          '\\s+-(?!' + this.getKeywords(operators) + ')[a-zA-Z_][a-zA-Z0-9_]*',
          'gmi'
        ),
        css: 'color1',
      }, // parameters
