...
;(function () {
  ...
  Music.prototype = {
    ...
    pageSize: 10,
    dataUrl:
      'https://tingapi.b0.upaiyun.com/v1/restserver/ting?method=baidu.ting.search.common',
    ...
    _buildMusicHtml: function (playerUrl) {
      var html =
        '<embed class="BDE_try_Music" allowfullscreen="false" pluginspage="http://www.macromedia.com/go/getflashplayer"'
      html += ' src="' + playerUrl + '"'
      html += ' width="1" height="1" style="position:absolute;left:-2000px;"'
      html +=
        ' type="application/x-shockwave-flash" wmode="transparent" play="true" loop="false"'
    ...
    _renderTemplate: function (data) {
      ...
      for (var i = 0, tmpList; (tmpList = data[i++]); ) {
        ...
        s.push(
          '<div class="m-h"><span class="m-t">' +
            lang.chapter +
            '</span><span class="m-s">' +
            lang.singer +
            '</span><span class="m-z">' +
            lang.special +
            '</span><span class="m-try-t">' +
            lang.listenTest +
            '</span></div>'
        )
        for (var j = 0, tmpObj; (tmpObj = tmpList[j++]); ) {
          ...
          s.push(
            '<input type="radio" id="radio-' +
              ...
              '-' +
              j +
              '" name="musicId" class="m-l" onclick="music.doselect(' +
              (me.pageSize * (i - 1) + (j - 1)) +
              ')"/>'
          )
          s.push(
            '<span class="m-t">' + me._getMaxText(tmpObj.title) + '</span>'
          )
          s.push(
            '<span class="m-s">' + me._getMaxText(tmpObj.author) + '</span>'
          )
          s.push(
            '<span class="m-z">' +
              me._getMaxText(tmpObj.album_title) +
              '</span>'
          )
          s.push(
            '<span class="m-try" onclick="music.doselect(' +
              (me.pageSize * (i - 1) + (j - 1)) +
              ');music.listenTest(this)"></span>'
          )
          s.push('</label>')
        }
        s.push('</div>')
