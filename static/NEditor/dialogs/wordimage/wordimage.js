...
function createFlashUploader(opt, callbacks) {
  ...
  for (var i in i18n) {
    if (
      !(i in { lang: 1, uploadingTF: 1, imageTF: 1, textEncoding: 1 }) &&
      i18n[i]
    ) {
      i18n[i] = encodeURIComponent(
        editor.options.langPath + editor.options.lang + '/images/' + i18n[i]
      )
    }
  }
